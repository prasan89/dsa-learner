...
import {
  ...
  type CategoryStatus,
  type AiTopic,
} from "@/lib/learn/ai-catalog";
import { AI_VISUALIZATIONS } from "@/components/learn/AiMiniViz";

// ── Types ─────────────────────────────────────────────────────────────────────
type TypeFilter = "all" | AiCategoryType;
...
// ── Card ──────────────────────────────────────────────────────────────────────
function AiEngineeringTopicCard({
  topic, index, status = "not-started", completedCount = 0,
}: {
  topic: AiTopic; index: number; status?: CategoryStatus; completedCount?: number;
}) {
  const [hovered, setHovered] = useState(false);
  const Viz = AI_VISUALIZATIONS[topic.id];
      ...
      <div className="p-5">
        ...
        <div className={`flex items-center justify-center min-h-[72px] mb-4 rounded-xl transition-colors ${
          ...
          {Viz ? (
            ...
            <div className="text-xs text-gray-300 font-mono">diagram</div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-gray-400">
              <Clock size={10} />
              {aiFormatDuration(topic.estimatedMinutes)}
            </span>
          </div>
          {topic.available && (
...
export default function AiEngineeringPage() {
  ...
  const countFor = (val: string) =>
    val === "all"
      ? AI_TOPICS.length
      : AI_TOPICS.filter(t => t.type === val).length;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">AI Engineering</h1>
          <p className="text-base text-gray-500 leading-relaxed max-w-xl">
            Build, evaluate and deploy production-grade AI applications.
          </p>
        </div>
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
          {[
            "Foundations", "LLMs", "Prompting", "Embeddings",
            "RAG", "Tools", "Agents", "Evaluation", "Production",
          ].map((step, i, arr) => (
            <React.Fragment key={step}>
              <span className="whitespace-nowrap text-[10px] font-semibold text-gray-500 bg-white border border-gray-200 rounded-full px-2.5 py-1">
                {step}
              </span>
              {i < arr.length - 1 && <span className="text-gray-300 text-xs shrink-0">→</span>}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {AI_CATEGORY_TABS.map(({ value, label }) => (
            <button
              key={value}
        ...
        {filtered.length === 0 ? (
          ...
          <ul className={
            ...
            {filtered.map((topic, i) => (
              <li key={topic.id}>
                <AiEngineeringTopicCard topic={topic} index={i + 1} />
              </li>
            ))}
          </ul>
        )}
