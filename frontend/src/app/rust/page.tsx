...
import {
  ...
  type CategoryStatus,
  type RustTopic,
} from "@/lib/learn/rust-catalog";
import { RUST_VISUALIZATIONS } from "@/components/learn/RustMiniViz";

// ── Types ─────────────────────────────────────────────────────────────────────
type TypeFilter = "all" | RustCategoryType;
...
// ── Card ──────────────────────────────────────────────────────────────────────
function RustTopicCard({
  topic, index, status = "not-started", completedCount = 0,
}: {
  topic: RustTopic; index: number; status?: CategoryStatus; completedCount?: number;
}) {
  const [hovered, setHovered] = useState(false);
  const Viz = RUST_VISUALIZATIONS[topic.id];
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-gray-400">
              <Clock size={10} />
              {rustFormatDuration(topic.estimatedMinutes)}
            </span>
          </div>
          {topic.available && (
...
export default function RustPage() {
  ...
  const countFor = (val: string) =>
    val === "all"
      ? RUST_TOPICS.length
      : RUST_TOPICS.filter(t => t.type === val).length;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">Rust Expert</h1>
          <p className="text-base text-gray-500 leading-relaxed max-w-xl">
            Master Rust from fundamentals to safe systems programming, concurrency and high-performance backend development.
          </p>
        </div>

        {/* ── Category Tabs ─────────────────────────────────────────────── */}
        <div className="flex flex-wrap gap-1.5">
          {RUST_CATEGORY_TABS.map(({ value, label }) => (
            <button
              key={value}
        ...
        {filtered.length === 0 ? (
          ...
          <ul className={
            ...
            {filtered.map((topic, i) => (
              <li key={topic.id}>
                <RustTopicCard topic={topic} index={i + 1} />
              </li>
            ))}
          </ul>
        )}
