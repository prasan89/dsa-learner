...
import {
  SYSDESIGN_TOPICS,
  SYSDESIGN_CATEGORY_TABS,
  sdFormatDuration,
  type SysDesignCategoryType,
  type CategoryStatus,
  type SysDesignTopic,
} from "@/lib/learn/sysdesign-catalog";
import { SYSDESIGN_VISUALIZATIONS } from "@/components/learn/SysDesignMiniViz";

// ── Types ─────────────────────────────────────────────────────────────────────
type TypeFilter = "all" | SysDesignCategoryType;
...
// ── Card ──────────────────────────────────────────────────────────────────────
function SysDesignTopicCard({
  topic, index, status = "not-started", completedCount = 0,
}: {
  topic: SysDesignTopic; index: number; status?: CategoryStatus; completedCount?: number;
}) {
  const [hovered, setHovered] = useState(false);
  const Viz = SYSDESIGN_VISUALIZATIONS[topic.id];
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-gray-400">
              <Clock size={10} />
              {sdFormatDuration(topic.estimatedMinutes)}
            </span>
          </div>
          {topic.available && (
...
export default function SystemDesignPage() {
  ...
  const countFor = (val: string) =>
    val === "all"
      ? SYSDESIGN_TOPICS.length
      : SYSDESIGN_TOPICS.filter(t => t.type === val).length;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">System Design Expert</h1>
          <p className="text-base text-gray-500 leading-relaxed max-w-xl">
            Learn to design scalable, reliable and high-performance systems.
          </p>
        </div>

        {/* ── Category Tabs ─────────────────────────────────────────────── */}
        <div className="flex flex-wrap gap-1.5">
          {SYSDESIGN_CATEGORY_TABS.map(({ value, label }) => (
            <button
              key={value}
        ...
        {filtered.length === 0 ? (
          ...
          <ul className={
            ...
            {filtered.map((topic, i) => (
              <li key={topic.id}>
                <SysDesignTopicCard topic={topic} index={i + 1} />
              </li>
            ))}
          </ul>
        )}
