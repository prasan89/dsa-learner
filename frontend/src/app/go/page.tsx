...
import {
  ...
  type CategoryStatus,
  type GoTopic,
} from "@/lib/learn/go-catalog";
import { GO_VISUALIZATIONS } from "@/components/learn/GoMiniViz";

// ── Types ─────────────────────────────────────────────────────────────────────
type TypeFilter = "all" | GoCategoryType;
...
// ── Card ──────────────────────────────────────────────────────────────────────
function GoTopicCard({
  topic, index, status = "not-started", completedCount = 0,
}: {
  topic: GoTopic; index: number; status?: CategoryStatus; completedCount?: number;
}) {
  const [hovered, setHovered] = useState(false);
  const Viz = GO_VISUALIZATIONS[topic.id];
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-gray-400">
              <Clock size={10} />
              {goFormatDuration(topic.estimatedMinutes)}
            </span>
          </div>
          {topic.available && (
...
export default function GoPage() {
  ...
  const countFor = (val: string) =>
    val === "all"
      ? GO_TOPICS.length
      : GO_TOPICS.filter(t => t.type === val).length;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">Go Expert</h1>
          <p className="text-base text-gray-500 leading-relaxed max-w-xl">
            Master Go from fundamentals to high-performance concurrent and cloud-native systems.
          </p>
        </div>

        {/* ── Category Tabs ─────────────────────────────────────────────── */}
        <div className="flex flex-wrap gap-1.5">
          {GO_CATEGORY_TABS.map(({ value, label }) => (
            <button
              key={value}
        ...
        {filtered.length === 0 ? (
          ...
          <ul className={
            ...
            {filtered.map((topic, i) => (
              <li key={topic.id}>
                <GoTopicCard topic={topic} index={i + 1} />
              </li>
            ))}
          </ul>
        )}
