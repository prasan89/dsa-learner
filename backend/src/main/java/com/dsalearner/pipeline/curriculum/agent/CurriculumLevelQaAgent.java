...
@Slf4j
public class CurriculumLevelQaAgent implements Agent<CurriculumQaInput, CurriculumQaResult> {
    ...
    public String agentType() {
        return AgentType.CURRICULUM_LEVEL_QA;
    }
    ...
    public AgentOutput<CurriculumQaResult> execute(AgentInput<CurriculumQaInput> input) {
        ...
        String userPrompt = prompt.promptText()
                ...
                .replace("{{cefrLevel}}", payload.cefrLevel() != null ? payload.cefrLevel() : "")
                .replace("{{deterministicCheckSummary}}", payload.deterministicCheckSummary() != null
                        ? payload.deterministicCheckSummary() : "")
                .replace("{{levelSummaryJson}}", payload.levelSummaryJson() != null
                        ? payload.levelSummaryJson() : "")
                .replace("{{curriculumSummaryJson}}", payload.curriculumSummaryJson() != null
                        ? payload.curriculumSummaryJson() : "{}");

        log.info("{}: evaluating level {} for {}", agentType(), payload.cefrLevel(), payload.languageDisplayName());
    ...
    private CurriculumQaResult parseResult(String text) {
        try {
            ...
            log.error("{}: failed to parse level QA JSON — {}", agentType(), e.getMessage());
